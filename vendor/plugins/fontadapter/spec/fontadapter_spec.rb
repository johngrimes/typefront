...
OTF_PATH = File.expand_path(File.dirname(__FILE__) + '/files/shouldve known.otf')
DROID_PATH = File.expand_path(File.dirname(__FILE__) + '/files/truetype/droid.TTF')
...
TEMP_WOFF_PATH = File.expand_path(File.dirname(__FILE__) + '/files/test.woff')
TEMP_TTF_PATH = File.expand_path(File.dirname(__FILE__) + '/files/test.ttf')
TEMP_OTF_PATH = File.expand_path(File.dirname(__FILE__) + '/files/test.otf')
TEMP_EOT_PATH = File.expand_path(File.dirname(__FILE__) + '/files/test.eot')
TEMP_SVG_PATH = File.expand_path(File.dirname(__FILE__) + '/files/test.svg')
...
describe 'FontAdapter' do
  ...
  it 'should convert a TrueType font file to OpenType and then back again' do
    ttf_adapter = FontAdapter.new(TTF_PATH)
    ttf_adapter.to_otf(TEMP_OTF_PATH)

    otf_adapter = FontAdapter.new(TEMP_OTF_PATH)
    otf_adapter.format.should == FontAdapter::OTF
    ttf = otf_adapter.to_ttf(TEMP_TTF_PATH)

    ttf_adapter = FontAdapter.new(TEMP_TTF_PATH)
    ttf_adapter.format.should == FontAdapter::TTF
  ...
  it 'should convert a TrueType font file to a WOFF and then back again' do
    ...
    ttf = woff_adapter.to_ttf(TEMP_TTF_PATH)
  ...
  it 'should convert a WOFF to an SVG' do
    woff_adapter = FontAdapter.new(WOFF_PATH)
    woff_adapter.to_svg(TEMP_SVG_PATH)
    File.exists?(TEMP_SVG_PATH).should == true

    FileUtils.rm TEMP_SVG_PATH
  end
  
  it 'should convert a OpenType font file to an SVG' do
    otf_adapter = FontAdapter.new(OTF_PATH)
    otf_adapter.to_svg(TEMP_SVG_PATH)
    File.exists?(TEMP_SVG_PATH).should == true

    FileUtils.rm TEMP_SVG_PATH
  end

  it 'should convert a TrueType font file to an SVG' do
    ttf_adapter = FontAdapter.new(TTF_PATH)
    ttf_adapter.to_svg(TEMP_SVG_PATH)
    File.exists?(TEMP_SVG_PATH).should == true
  ...
  it 'should extract correct name fields for Droid Sans' do
    fontadapter = FontAdapter.new(DROID_PATH)
